...
import { ProductFormSchema, ProductFormType } from "@/lib/validation";
...
export default function ProductForm({ initalData }: { initalData?: Product }) {
  ...
  async function onsubmit(data: ProductFormType) {
    if (data.Files.length < 2) {
      setError("Files", { message: "Please upload at least two image" });
      return;
    }
    if (data.SubCategoryNames.length === 0) {
      setError("SubCategoryNames", {
        message: "Please Write At Least Two Tags",
      });
      return;
    }
